 ...
 */
const themeLocalSearch = function ({
  search_path,
  zip_Path,
  version_Path,
  input_Trigger,
    ...
    var inputEventFunction = function () {
      ...
      if (searchText.length > 0) {
        ...
        datas.forEach(function (data) {
          ...
          var content = data.content ? data.content.trim().replace(
            /<[^>]+>/g, "") : '';
          ...
          keywords.forEach(function (keyword) {
            ...
            indexOfTitle = indexOfTitle.concat(getIndexByWord(
              keyword, titleInLowerCase, false));
            indexOfContent = indexOfContent.concat(getIndexByWord(
              keyword, contentInLowerCase, false));
          });
          ...
          if (isMatch) {
            // sort index by position of keyword
              ...
              index.sort(function (itemLeft, itemRight) {
                if (itemRight.position !== itemLeft
                  .position) {
                  return itemRight.position - itemLeft
                    .position;
                } else {
                  return itemLeft.word.length - itemRight.word
                    .length;
                }
              });
            });
            ...
            while (indexOfContent.length != 0) {
              ...
              slicesOfContent.push(mergeIntoSlice(content, start, end,
                indexOfContent));
            }
            // sort slices in content by search text's count and hits' count
            slicesOfContent.sort(function (sliceLeft, sliceRight) {
              if (sliceLeft.searchTextCount !== sliceRight
                .searchTextCount) {
                return sliceRight.searchTextCount - sliceLeft
                  .searchTextCount;
              } else if (sliceLeft.hits.length !== sliceRight.hits
                .length) {
            ...
            function highlightKeyword(text, slice) {
              ...
              slice.hits.forEach(function (hit) {
                ...
                result += '<b class="search-keyword">' + text
                  .substring(hit.position, end) + '</b>';
                prevEnd = end;
              });
            ...
            if (slicesOfTitle.length != 0) {
              resultItem += "<li><a target='_blank' href='" +
                articleUrl + "' class='search-result-title'>" +
                highlightKeyword(title, slicesOfTitle[0]) + "</a>";
            } else {
              resultItem += "<li><a target='_blank' href='" +
                articleUrl + "' class='search-result-title'>" +
                title + "</a>";
            ...
            slicesOfContent.forEach(function (slice) {
              resultItem +=
                ...
                highlightKeyword(content, slice) + "...</p>";
            });
            resultItem += "</li>";
            resultItems.push({
              item: resultItem,
      ...
      if (keywords.length === 1 && keywords[0] === "") {
        resultContent.innerHTML =
          '<div id="no-result"><i class="" />Please type in some words!</div>'
      } else if (resultItems.length === 0) {
        resultContent.innerHTML =
          '<div id="no-result"><i class="" />No any results!</div>'
      } else {
        ...
        var searchResultList = '<div class=\"search-result-number"\>' +
          resultItems.length + ' results at total!</div>' +
