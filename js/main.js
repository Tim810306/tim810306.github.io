  ...
  const Annie_Preloader = function () {
    ...
    function singlePageScroll() {
      if (browserRefreshStatus) {
        currentScrollHeight = currentScrollTop || PRELOADER.getCookie(
          'currentScrollTop');
    ...
    if (CONFIG_BGIMAGE.preloaderEnable && CONFIG_BGIMAGE
      .preloaderEnable != null) { // 不设置预加载
      // 10s以后
      ...
      20); // delayTime = ANNIE.delayTimeG * 20 = 10s
  ...
  const Annie_Progress = function () {
    ...
    $(window).scroll(function () {
      ...
      if ($(postPageId).length) {
        ...
        scrollPercent = ((scrollTop - headerH) / (postContentH +
          postTitleH + postMetaH + 80 - windowHeight)) * 100;
      } else {
        scrollPercent = (scrollTop / (docHeight - windowHeight)) *
        100;
      }
      ...
      $("#progress-bar").attr("style", "width:" + (scrollPercent) +
        "%; display: block;");
      if (scrollTop >= ((scrollLimit > headerH) ? scrollLimit :
          headerH)) {
        $(navBarId).css({
          top: '0'
        }).show();
    ...
    function fixedAndShowTocId() {
      $(window).scroll(function () {
        ...
        if ((scrollTop >= upperLimit1) && (scrollTop < upperLimit2)) {
          //屏幕宽度<= 1024px时应隐藏
          $(postTocId).css('position', 'fixed').show().fadeIn(
            delayTime);
        } else {
  ...
  const Annie_Archive = function () {
    ...
    $("#layout-archive-year  ul li").each(function (i) {
      ...
      if (year < currentYear || currentYear == "") {
        ...
        $(this).before("<h3 class = '" + currentYear + "'>" +
          currentYear + "&nbsp;&nbsp;" + "<i class='icon-" +
          getZodiac(year) + "'></i>" + "<em>(" + $("[year = '" +
            currentYear + "']").length + "篇)</em>" + "</h3>");
      }
      $(this).attr("year", currentYear);
    });
    $("#layout-archive-year h3").each(function () {
      $("#layout-archive-year ul li[year = '" + $(this).attr(
        "class") + "'").wrapAll("<div year = '" + $(this).attr(
        "class") + "'></div>");
    ...
    $('body').on('click', paginationId, function () {
      ...
      $.ajax({
        ...
        error: function (event, xhr, options) {
          $(paginationId).attr("href", thisUrl).empty().text(
            loaderTitle);
          alert("Error requesting " + options.url + ":  " + xhr
            .status + ",  " + xhr.statusText);
          console.log("Error requesting " + options.url +
            ":  " + xhr.status + ",  " + xhr.statusText)
        },
        success: function (data) {
          let result = $(data).find("#post"),
            newhref = $(data).find(paginationId).attr("href");
          $(paginationContainer).append(result.fadeIn(delayTime)
            .addClass('animation-zoom'));
          if (leancloudCount) {
            //FIX: ajax bug
          ...
          if (newhref != undefined) {
            ...
            $("#pagination").html("<span>" + loaderStatus +
              "</span>");
          }
        },
        complete: function () {
      ...
      if (href != undefined) {
        $.ajax({
          ...
          success: function (data) {
            ...
            $('#TCP-content').append(result.fadeIn(delayTime)
              .addClass('animation-zoom'));
            $(".layout-archive").css({
              'paddingTop': '0'
            });
...
console.log("%c Github %c", "background: #222222; color: #ffffff", " ",
  "https://github.com/Sariay/hexo-theme-Annie");
