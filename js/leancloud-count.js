...
function init(currentCounter, currentPost) {
  ...
  newCounter.save().then(function (newCounter) {
    ...
    console.error("Failed to create new object, with error message: " +
      error.message + " / " + currentPost.title);
  });
}
      ...
      if (results.length > 0) {
        ...
        thisPost.save().then(
          function (thisPost) {
            ...
            $(document.getElementById(currentPost.visitorNumberContainer))
              .text(content);
            console.log('Succed to add visitors.');
            console.log('New visitors: ' + thisPost.id + " " + thisPost.get(
              'url') + " " + thisPost.get('visitors'));
          },
          function (error) {
            // 异常处理
      ...
      if (results.length > 0) {
        ...
        thisPost.save().then(
          function (thisPost) {
            ...
            $(document.getElementById(currentPost.likeNumberContainer))
              .text(content);
            console.log('Succed to add likes.');
            console.log('New likes: ' + thisPost.id + " " + thisPost.get(
              'url') + " " + thisPost.get('likes'));
          },
          function (error) {
            // 异常处理
...
function topNPost(limitCount) {
  ...
  query.find().then(
    function (results) {
      let smallerValue = (limitCount > results.length) ? results.length :
        limitCount;
      if (smallerValue > 0) {
        for (let i = 0; i < smallerValue; i++) {
          ...
          let topNumber = i + 1,
            topContent = "<ul class='topN-post-list'>" +
            "<li class='topN-post-item'>" + "<span class='item-topNumber'>" +
            "TOP-" + topNumber + "</span>" +
            "<a class='item-title' target='_blank' href='" + url + "'>" +
            title + "</a>" + "<i class='item-visitors'>" + "「文章热度:" +
            visitors + "℃」" + "</i>" + "</li>" + "</ul>";
...
const post = {
  url: ($(".leancloud_likes").attr('data-url') || $(".leancloud_visitors")
    .attr('data-url')) ? ($(".leancloud_likes").attr('data-url').trim() ||
    $(".leancloud_visitors").attr('data-url').trim()) : " ",
  title: ($(".leancloud_likes").attr('data-title') || $(".leancloud_visitors")
    .attr('data-title')) ? ($(".leancloud_likes").attr('data-title')
  .trim() || $(".leancloud_visitors").attr('data-title').trim()) : " ",
  likes: 0,
  visitors: 1,
  visitorNumberContainer: $(".leancloud_visitors_count").attr('id') ? $(
    ".leancloud_visitors_count").attr('id').trim() : " ",
  likeNumberContainer: $(".leancloud_likes_count").attr('id') ? $(
    ".leancloud_likes_count").attr('id').trim() : " "
}
let urlCheck =
...
if (urlCheckStatus) {
  ...
  console.log("The format of <" + CONFIG_LEACLOUD_COUNT.serverURLs +
    "> is not correct!");
}
...
if (($('.layout-pure').length || $('.layout-cart').length) && $('.post-title')
  .length >= 1) {
  //主页: 只展示次数
