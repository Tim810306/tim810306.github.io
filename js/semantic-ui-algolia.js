function semanticUiAlgolia() {
  ...
  function renderHit(hit) {
    var result = '';
    result = result +
      '<div class="item"><div class="content"><a class="header" href="/' + hit
      .path + '">';
  ...
  function renderTags(tags, tagsRaw) {
    ...
    for (var i = 0; i < tags.length; i++) {
      ...
      if (string.indexOf('<span class="highlight">') != -1) {
        var temp = string.replace('<span class="highlight">', '').replace(
          '</span>', '');
        result = result + '<a class="ui mini blue label" href="' + tagRaw.path +
          '">' + temp + '</a>';
      } else {
        result = result + '<a class="ui mini label" href="' + tagRaw.path +
          '">' + string + '</a>';
      }
    }
    return result;
